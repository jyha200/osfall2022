...
* Assigned: 2022-11-11 Friday
* **Due: 2022-11-26 Saturday midnight


* Please use the [Help Document for Project 3](https://github.com/jyha200/osfall2022/blob/master/presentations/2022%20OS%20FALL%20Project3%20Help%20Document.pdf) as reference 
...
Start from the original [kernel source](https://github.com/jyha200/tizen-5.0-rpi3) (you may already have it in your team repository's _master_ branch) and make incremental changes to get things done.
...
Moreover, we provide a daemon that updates this fake device rotation information, called [rotd](https://github.com/jyha200/osfall2022/blob/master/src/proj3/rotd.c), for you to save time. :)
...
```
Please make sure `/dev/vdd` is the partition mounted to `/`. You can check this by typing `df`.
If you want to make this change permanent, simply edit `/etc/fstab` and remove the `ro` option.

## Others
    - Submit (slides, demo video, audio recording, diff) to etl before the deadline
    - Title: [OS-ProjX] TeamX slides&demo submission
    - File name: TeamX-slides.ppt(.pdf), TeamX-demo.mp4(.avi….)(send only one video!), TeamX-audio.mp3(...), TeamX.diff
